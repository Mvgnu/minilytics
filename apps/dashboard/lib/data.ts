...
function safeProperties(value: unknown): Record<string, string | number | boolean | null> {
  ...
  for (const [key, item] of Object.entries(value as Record<string, unknown>)) {
    if (item === null || typeof item === "string" || typeof item === "number" || typeof item === "boolean") {
      result[key.slice(0, 128)] = item;
    }
  }
  return Buffer.byteLength(JSON.stringify(result)) <= 4096 ? result : {};
}
...
function classifySource(input: { referrer: string; utmSource: string; utmMedium: string; siteDomain: string }) {
  const { referrer, utmSource, utmMedium, siteDomain } = input;
  if (utmSource || utmMedium) return { source: "campaign", medium: utmMedium || "campaign", detail: utmSource || null };
  ...
  try {
    ...
    if (host === ownHost || host.endsWith(`.${ownHost}`)) return { source: "internal", medium: "internal", detail: host };
    const search: Array<[RegExp, string]> = [
      [/(^|\.)google\./, "google"], [/(^|\.)bing\.com$/, "bing"], [/(^|\.)duckduckgo\.com$/, "duckduckgo"],
      [/(^|\.)ecosia\.org$/, "ecosia"], [/(^|\.)yahoo\./, "yahoo"], [/(^|\.)yandex\./, "yandex"],
    ];
    for (const [pattern, name] of search) if (pattern.test(host)) return { source: "organic", medium: "search", detail: name };
    ...
    for (const [pattern, name] of social) if (pattern.test(host)) return { source: "social", medium: "social", detail: name };
...
export async function getSitesOverview(days = 30) {
  ...
  return sql<{ id: string; name: string; domain: string; visitors: number; pageviews: number; events: number }[]>`
    ...
    FROM sites s
    LEFT JOIN events e ON e.site_id = s.id AND e.occurred_at >= now() - ${days} * interval '1 day'
    GROUP BY s.id, s.name, s.domain ORDER BY visitors DESC, s.name ASC
  `;
}

export async function getSiteDashboard(siteId: string, days = 30) {
  const sql = db();
  const [siteRow] = await sql<{ id: string; name: string; domain: string; keyEvents: unknown; funnels: unknown }[]>`
    SELECT id, name, domain, key_events AS "keyEvents", funnels FROM sites WHERE id = ${siteId} LIMIT 1
  `;
  if (!siteRow) return null;
  const site = { id: siteRow.id, name: siteRow.name, domain: siteRow.domain, keyEvents: parseKeyEvents(siteRow.keyEvents) };
  ...
  const [summary] = await sql<{
    ...
    WITH session_rollup AS (
      ...
      WHERE e.site_id = ${siteId} AND e.occurred_at >= now() - ${days} * interval '1 day'
      GROUP BY e.session_id
    )
    SELECT COUNT(DISTINCT visitor_key)::int AS visitors, COUNT(*)::int AS sessions,
      COUNT(*) FILTER (WHERE engagement_ms >= 10000 OR pageviews >= 2 OR key_events > 0)::int AS "engagedSessions",
      CASE WHEN COUNT(*) = 0 THEN 0 ELSE (100.0 * COUNT(*) FILTER (WHERE engagement_ms >= 10000 OR pageviews >= 2 OR key_events > 0) / COUNT(*))::double precision END AS "engagementRate",
      CASE WHEN COUNT(*) = 0 THEN 0 ELSE (100.0 - 100.0 * COUNT(*) FILTER (WHERE engagement_ms >= 10000 OR pageviews >= 2 OR key_events > 0) / COUNT(*))::double precision END AS "bounceRate",
      ...
      CASE WHEN COUNT(*) = 0 THEN 0 ELSE (100.0 * COUNT(*) FILTER (WHERE key_events > 0) / COUNT(*))::double precision END AS "keyEventRate",
  ...
  const traffic = await sql<{ day: string; visitors: number; pageviews: number }[]>`
    ...
    FROM events WHERE site_id = ${siteId} AND occurred_at >= now() - ${days - 1} * interval '1 day'
    GROUP BY date_trunc('day', occurred_at) ORDER BY date_trunc('day', occurred_at)
  `;

  const pages = await sql<{ path: string; views: number; visitors: number; clicks: number; avgEngagementMs: number }[]>`
    SELECT path,
    ...
    FROM events WHERE site_id = ${siteId} AND occurred_at >= now() - ${days} * interval '1 day'
    GROUP BY path HAVING COUNT(*) FILTER (WHERE event_type = 'pageview') > 0 ORDER BY views DESC LIMIT 15
  `;

  const goals = await sql<{ eventType: string; count: number; sessions: number; visitors: number }[]>`
    SELECT e.event_type AS "eventType", COUNT(*)::int AS count, COUNT(DISTINCT e.session_id)::int AS sessions,
      COUNT(DISTINCT COALESCE(e.visitor_id, e.session_id))::int AS visitors
    FROM events e JOIN sites s ON s.id = e.site_id
    WHERE e.site_id = ${siteId} AND e.occurred_at >= now() - ${days} * interval '1 day' AND s.key_events ? e.event_type
    GROUP BY e.event_type ORDER BY count DESC
  `;

  const sessionAcquisition = await sql<{
    source: string; detail: string | null; campaign: string | null; sessions: number; engagedSessions: number; keyEventSessions: number;
  }[]>`
    WITH rollup AS (
      SELECT e.session_id, MIN(e.source) AS source, MIN(e.source_detail) AS detail, MIN(e.campaign) AS campaign,
        COUNT(*) FILTER (WHERE e.event_type = 'pageview')::int AS pageviews,
        COALESCE(SUM(CASE WHEN e.event_type = 'engagement' AND COALESCE(e.properties->>'engagementMs', '') ~ '^[0-9]+(?:\\.[0-9]+)?$'
          THEN (e.properties->>'engagementMs')::double precision ELSE 0 END), 0)::double precision AS engagement_ms,
        COUNT(*) FILTER (WHERE s.key_events ? e.event_type)::int AS key_events
    ...
    SELECT source, detail, campaign, COUNT(*)::int AS sessions,
      COUNT(*) FILTER (WHERE engagement_ms >= 10000 OR pageviews >= 2 OR key_events > 0)::int AS "engagedSessions",
      COUNT(*) FILTER (WHERE key_events > 0)::int AS "keyEventSessions"
    FROM rollup GROUP BY source, detail, campaign ORDER BY sessions DESC LIMIT 15
  `;

  const userAcquisition = await sql<{ source: string; detail: string | null; campaign: string | null; visitors: number }[]>`
    WITH period_visitors AS (
      SELECT DISTINCT visitor_id FROM events
      WHERE site_id = ${siteId} AND visitor_id IS NOT NULL AND occurred_at >= now() - ${days} * interval '1 day'
    ), first_touch AS (
      SELECT DISTINCT ON (e.visitor_id) e.visitor_id, e.source, e.source_detail AS detail, e.campaign
      FROM events e JOIN period_visitors p ON p.visitor_id = e.visitor_id
      WHERE e.site_id = ${siteId} ORDER BY e.visitor_id, e.occurred_at ASC, e.id ASC
    )
    SELECT source, detail, campaign, COUNT(*)::int AS visitors FROM first_touch
    GROUP BY source, detail, campaign ORDER BY visitors DESC LIMIT 15
  `;

  const landingPages = await sql<{ path: string; sessions: number; engagedSessions: number; keyEventSessions: number }[]>`
    WITH rollup AS (
      SELECT e.session_id, COUNT(*) FILTER (WHERE e.event_type = 'pageview')::int AS pageviews,
        COALESCE(SUM(CASE WHEN e.event_type = 'engagement' AND COALESCE(e.properties->>'engagementMs', '') ~ '^[0-9]+(?:\\.[0-9]+)?$'
          THEN (e.properties->>'engagementMs')::double precision ELSE 0 END), 0)::double precision AS engagement_ms,
        COUNT(*) FILTER (WHERE s.key_events ? e.event_type)::int AS key_events
      FROM events e JOIN sites s ON s.id = e.site_id
      WHERE e.site_id = ${siteId} AND e.occurred_at >= now() - ${days} * interval '1 day' GROUP BY e.session_id
    ), landing AS (
      SELECT DISTINCT ON (session_id) session_id, path FROM events
      WHERE site_id = ${siteId} AND event_type = 'pageview' AND occurred_at >= now() - ${days} * interval '1 day'
    ...
    FROM landing JOIN rollup USING (session_id) GROUP BY landing.path ORDER BY sessions DESC LIMIT 15
  `;
  ...
  const exitPages = await sql<{ path: string; exits: number }[]>`
    ...
    SELECT path, COUNT(*)::int AS exits FROM exits GROUP BY path ORDER BY exits DESC LIMIT 15
  `;

  const webVitals = await sql<{ metric: string; p75: number; samples: number; goodPercent: number }[]>`
    ...
    FROM events
    WHERE site_id = ${siteId} AND event_type = 'web_vital' AND occurred_at >= now() - ${days} * interval '1 day'
  ...
  const events = await sql<{ eventType: string; count: number }[]>`
    SELECT event_type AS "eventType", COUNT(*)::int AS count FROM events
    WHERE site_id = ${siteId} AND event_type NOT IN ('pageview', 'engagement', 'web_vital')
      AND occurred_at >= now() - ${days} * interval '1 day'
    GROUP BY event_type ORDER BY count DESC LIMIT 15
  `;

  const recentRows = await sql<JourneyEvent[]>`
    SELECT session_id AS "sessionId", event_type AS "eventType", path, source, source_detail AS detail,
      target_label AS "targetLabel", target_url AS "targetUrl", occurred_at AS "occurredAt"
    FROM events WHERE site_id = ${siteId} AND event_type NOT IN ('engagement', 'web_vital')
      AND occurred_at >= now() - ${days} * interval '1 day'
    ORDER BY occurred_at DESC LIMIT 300
  `;

  const journeys = new Map<string, { sessionId: string; source: string; detail: string | null; events: JourneyEvent[] }>();
  for (const row of recentRows) {
    const existing = journeys.get(row.sessionId);
    if (existing) {
      existing.events.push(row);
      continue;
    }
    if (journeys.size >= 8) continue;
    journeys.set(row.sessionId, { sessionId: row.sessionId, source: row.source, detail: row.detail, events: [row] });
  }
  for (const journey of journeys.values()) {
    journey.events.sort((a, b) => new Date(a.occurredAt).getTime() - new Date(b.occurredAt).getTime());
  }

  let configuredFunnels: ReturnType<typeof evaluateFunnels> = [];
  if (funnelDefinitions.length) {
    const funnelRows = await sql<FunnelEvent[]>`
      SELECT session_id AS "sessionId", event_type AS "eventType", path, target_label AS "targetLabel", occurred_at AS "occurredAt"
      FROM events WHERE site_id = ${siteId} AND event_type NOT IN ('engagement', 'web_vital')
        AND occurred_at >= now() - ${days} * interval '1 day'
  ...
  const safeSummary = summary ?? {
    visitors: 0, sessions: 0, engagedSessions: 0, engagementRate: 0, bounceRate: 0, pageviews: 0,
    pagesPerSession: 0, avgEngagementMs: 0, keyEventCount: 0, keyEventSessions: 0, keyEventRate: 0, trackedEvents: 0,
  };
  const sessionFunnel = {
    id: "session-quality", name: "Session funnel",
    steps: [
  ...
  return {
    site, summary: safeSummary, traffic, pages, goals, sessionAcquisition, userAcquisition,
    landingPages, exitPages, webVitals, events, journeys: [...journeys.values()], funnels: [sessionFunnel, ...configuredFunnels],
  };
}
